...
exports.default = function (data, schema, options) {
  ...
  for (var i = 1; i < data.length; i++) {
    var result = read(schema, data[i], i - 1, columns, errors);
    if (result) {
      results.push(result);
    }
  }
    ...
    if (isNestedSchema) {
      ...
      if (rawValue === null) {
        ...
        var array = parseArray(rawValue).map(function (_value) {
          var result = parseValue(_value, schemaEntry);
          if (result.error) {
            value = _value;
            error = result.error;
          }
        ...
        var result = parseValue(rawValue, schemaEntry);
        error = result.error;
...
function parseValueOfType(value, type) {
  switch (type) {
    ...
    case Date:
      ...
      var date = (0, _parseDate2.default)(value);
      if (!date) {
        return { error: 'invalid' };
      }
      return { value: date };
